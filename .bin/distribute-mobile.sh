...
if [[ $4 == "ios" ]]
then
    currentVersion=$(grep -A1 'CFBundleShortVersionString' ios/mobile/Info.plist | tail -1 | grep -o '[0-9.]\+')
else
...
then
    ...
    if [[ $4 == "ios" ]]
    then
        appcenter codepush release-react -a $1 -d $2 --disable-duplicate-release-error --plist-file ./ios/mobile/Info.plist
    else
