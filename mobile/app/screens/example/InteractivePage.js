...
const TheComponent = class extends Component {
	...
	render() {
		const fade = this._deltaY.interpolate({
			inputRange: [-FILTER_HEIGHT, -FILTER_HEIGHT * .8, 0],
			outputRange: [0, 0, 1],
			extrapolateRight: 'clamp'
		});
					...
					<Interactable.View
						...
						<View style={styles.content}>
							<FormGroup style={{marginTop: -40}}>
								<TouchableOpacity style={[Styles.circleButton, {alignSelf: 'center'}]}
												  onPress={this.toggleFilter}>
									{
										this.state.expanded ? (
											<Fade autostart={true} value={1}>
												<ION style={Styles.circleButtonText}
													 name={"ios-arrow-down"}/>

											</Fade>
										) : <View><Fade autostart={true} value={1}>
											<ION style={Styles.circleButtonText}
												 name={"ios-arrow-up"}/>
										</Fade></View>
									}
...
const styles = StyleSheet.create({
	...
	swipeRow: {
		...
		borderBottomWidth: 1 / PixelRatio.get(), borderTopWidth: 1 / PixelRatio.get(), borderColor: '#d1d1d1'
	},
	panelButtonTitle: {
		fontSize: 17,
		fontWeight: 'bold',
