...
const SelectBox = class extends PureComponent {
  ...
  onPress = () => {
    ...
    API.showOptions(title, options, true, false, null, true)
      .then((i) => {
        if (onBlur) onBlur();
        if (onChange && i != null) onChange(i < options.length ? options[i] : null);
      });
  }

  render() {
    const { disabled, onPress, textStyle, containerStyle, iconStyle, hideIcon, children, title } = this.props;
    return (
