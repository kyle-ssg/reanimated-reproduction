...
export const waitForElementVisible = async (id)=> {
  try {
    ...
    waitFor(element(by.id(id))).toBeVisible().withTimeout(30000);
  }
}
export const waitByType = (theType)=> waitFor(element(by.type(theType))).toBeVisible().withTimeout(30000);
...
export const scrollWaitElementVisible = async (id,scrollId, direction="down") => {
  try {
    await waitFor(element(by.id(id))).toBeVisible()
      .whileElement(by.id(scrollId))
      .scroll(200, 'down');
...
export const mockError = async (data,url,namespace) => {
  await fetch(`http://localhost:5000?error=1&url=${url}&baseUrl=${Project.api}&namespace=${namespace||"default"}`,{
    method:"post",
    headers: {
      "Content-Type":"application/json"
    },
