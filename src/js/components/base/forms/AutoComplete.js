...
const AutoComplete = class extends React.Component {
	...
	onKeyUp = (e) => {
		if (Utils.keys.isBackspace(e) && this.props.value && this.props.value.length) {
	...
	addItem = (item) => {
		if (!this.props.multiple) {
			...
			this.refs.input.refs.input && this.refs.input.refs.input.refs.input.blur(); //this isn't ideal
			setTimeout(() => {
				this.setState({isFocused: false});
				this.refs.focus && this.refs.focus.toggle();
			}, 50)
			return
		}
									...
									<div className={classNames}>
										<div>
											<ListView
												...
												renderRow={(row, index) =>
													<div className="autocomplete-item" key={row.id || row}
														 onMouseDown={(e) => !e.nativeEvent.button && this.addItem(row)}>
